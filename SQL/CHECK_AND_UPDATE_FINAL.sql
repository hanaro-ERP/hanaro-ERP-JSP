DROP PROCEDURE CHECK_AND_UPDATE_FINAL;

DELIMITER $$
CREATE PROCEDURE CHECK_AND_UPDATE_FINAL(
...
BEGIN
    ...
    IF p_isCompleted = 0 AND p_repaymentAmount <= (SELECT account_balance FROM accounts WHERE a_id = p_accountId) THEN
        ...
        SET completed = 1
        WHERE lc_id = p_loanContractId and balance <= 0;
	ELSEIF p_isCompleted = 0 THEN
		UPDATE loanContracts
		SET delinquent_amount = p_repaymentAmount, delinquent_count = delinquent_count + 1
		WHERE lc_id = p_loanContractId;
	END IF;
